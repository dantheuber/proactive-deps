...
describe('DependencyMonitor', () => {
  ...
  it('registers and updates gauges using injected registry', async () => {
    ...
    expect(metrics).toMatch(
      /dependency_health\{dependency="redis",impact="Cache latency may increase"} 0/,
    );
    expect(metrics).toMatch(
      /dependency_health\{dependency="api",impact="Some features degraded"} 1/,
    );
  });
  describe('prom-client integration', () => {
    it('handles repeated metrics calls (init short-circuit)', async () => {
      ...
      expect(second).toContain(
        'dependency_health{dependency="svc",impact="Degraded responses"} 0',
      );
    });
  });
